...
//		bartop = (int) getResources().getDimension(R.dimen.px2dp_66);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		{
		int width = (perBarWidth + barPadding) * ((scrollBarBean.getData().size()<=7?7:scrollBarBean.getData().size()));// this的宽度
		...
		for (int i = 0; i < scrollBarBean.getData().size(); i++) {
			...
			perBarBean.setActualHeight((int)(perBarBean.getIcDateFloat()
...
//						&& y > perBarBean.getTopY() && y < height) {//控制点击高度的，不需要刻意不谢
						 && y < height) {
					perBarBean.setFlag(true);
